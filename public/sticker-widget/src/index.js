...
let PACKS_BASE_URL = `http://localhost:3000/stickers?user=${user}`
...
class App extends Component {
	...
	_loadPacks(disableCache = false) {
		...
		fetch(`${PACKS_BASE_URL}&type=index`, { cache }).then(async indexRes => {
			...
			for (const packFile of indexData.packs) {
				...
				const packRes = await fetch(`${PACKS_BASE_URL}&type=packs&pack=${packFile}`, { cache })
